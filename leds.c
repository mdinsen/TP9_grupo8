    ...
    while(c != 'q'){                          //Al presionar 'q' se sale del programa
        if(c == '\n'){
            if(esNumero(anterior)){
                ...
                BITSET_A(n);                  //Encendemos el bit "n" del puerto A que ingresó el usuario, en caso de que haya ingresado un numero valido: del 0 al 7
            }else{
                switch(anterior){
                    case 't':
                        MASKTOGGLE_A(0xFF);   //Con 't' cambiamos el estado de todos los bits del puerto A: si eran 0s pasan a ser 1s y viceversa
                        break;
                    case 'c':
                        MASKOFF_A(0xFF);      //Con 'c' se "apagan" todos los bits: pasan a ser todos 0
                        break;
                    case 's':
                        MASKON_A(0xFF);       //Con 's' se "encienden" todos los bits: pasan a ser todos 1
                        break;
                    default:
